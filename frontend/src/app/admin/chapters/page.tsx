  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-2xl font-bold text-slate-900">Chapters Management</h2>
          <p className="text-slate-500 mt-1">Organize syllabus chapters under subjects</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        ...
        <div className="md:col-span-2 relative h-16 md:h-auto">
          ...
          <input
            ...
            className="w-full h-full min-h-[4rem] pl-11 pr-4 rounded-xl border border-slate-200 focus:border-primary-500 focus:ring-2 focus:ring-primary-500/20 transition-all outline-none text-slate-900"
          />
        </div>
        ...
        <div className="h-16 md:h-auto">
          <select
            ...
            className="w-full h-full min-h-[4rem] px-4 rounded-xl border border-slate-200 focus:border-primary-500 focus:ring-2 focus:ring-primary-500/20 transition-all outline-none text-slate-900 bg-white"
          >
              ...
              {isLoading ? (
                <tr>
                  <td colSpan={6} className="px-6 py-12 text-center">
                    ...
                    <p className="mt-2 text-slate-500">Loading chapters...</p>
                  </td>
                </tr>
                ...
                data?.chapters.map((chapter: any) => (
                  <tr key={chapter._id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      {chapter.subject ? (
                        <div className="flex items-center gap-2">
                          ...
                          ></span>
                          <span className="text-slate-700">{chapter.subject.name}</span>
                        </div>
                      ) : (
                        <span className="text-red-500">Unknown Subject</span>
                      )}
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <button 
                        ...
                        className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 ${chapter.isActive ? 'bg-primary-600' : 'bg-slate-200'}`}
                      >
