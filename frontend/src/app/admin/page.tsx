...
export default function AdminDashboardPage() {
  ...
  const activityColumns = [
    { key: 'action', header: 'Action', cell: (item: Activity) => <span className="font-medium text-foreground">{item.action}</span> },
    { key: 'target', header: 'Target', cell: (item: Activity) => <span className="text-muted-foreground">{item.target}</span> },
    { key: 'user', header: 'User', cell: (item: Activity) => <span className="text-muted-foreground">{item.user}</span> },
    { key: 'date', header: 'Date', cell: (item: Activity) => <span className="text-muted-foreground text-sm">{item.date}</span> },
    {
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="text-3xl font-bold text-foreground tracking-tight">
            Welcome <span className="font-normal text-muted-foreground">{user?.fullName?.split(' ')[0] || 'Admin'}!</span>
          </h1>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1">
          <Card className="h-full flex flex-col">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-lg font-bold text-foreground">Exam Overview</h3>
              <select className="bg-muted border border-border rounded-full px-3 py-1 text-sm text-muted-foreground outline-none">
            ...
            <div className="flex-1 flex flex-col items-center justify-center relative my-4">
              ...
              <div className="absolute bottom-0 text-center">
                <p className="text-4xl font-bold text-foreground leading-none">{stats?.totalMockTests || 1200}</p>
                <p className="text-xs font-medium text-muted-foreground mt-1">Total Exam</p>
              </div>
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card>
          <h3 className="text-lg font-bold text-foreground mb-6">Subject Distribution</h3>
          {chartsLoading ? (
        ...
        <Card className="flex flex-col">
          <h3 className="text-lg font-bold text-foreground mb-6">Mock Tests Breakdown</h3>
          <div className="grid grid-cols-2 gap-4 flex-1">
            <div className="bg-muted p-4 rounded-2xl border border-border flex flex-col justify-center">
              <p className="text-xs font-medium text-muted-foreground mb-1">Full Mock Tests</p>
              <p className="text-xl font-bold text-foreground">{stats?.totalFullMockTests}</p>
            </div>
            <div className="bg-muted p-4 rounded-2xl border border-border flex flex-col justify-center">
              <p className="text-xs font-medium text-muted-foreground mb-1">Chapter-wise</p>
              <p className="text-xl font-bold text-foreground">{stats?.totalChapterWiseTests}</p>
            </div>
            <div className="bg-muted p-4 rounded-2xl border border-border flex flex-col justify-center">
              <p className="text-xs font-medium text-muted-foreground mb-1">PYQ Papers</p>
              <p className="text-xl font-bold text-foreground">{stats?.totalPyqPapers}</p>
            </div>
          </div>
        </Card>

        <Card className="flex flex-col justify-between">
          <h3 className="text-lg font-bold text-foreground mb-4">Questions</h3>
          <div className="flex gap-4">
            <div className="flex-1 bg-muted p-4 rounded-2xl border border-border flex items-center justify-between">
              <div>
                <p className="text-2xl font-bold text-foreground">{stats?.totalQuestions}</p>
                <p className="text-xs font-medium text-muted-foreground">Total QS</p>
              </div>
            ...
            <div className="flex-1 bg-muted p-4 rounded-2xl border border-border flex items-center justify-between">
              <div>
                ...
                <p className="text-xs font-medium text-muted-foreground">Questions Added</p>
              </div>
