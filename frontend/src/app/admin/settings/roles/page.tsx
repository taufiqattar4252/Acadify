    ...
    <div className="p-8">
      <div className="flex justify-between items-center border-b border-slate-100 pb-6 mb-8">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Review the access levels for different administrative roles.</p>
        </div>
      </div>
      ...
      {!isSuperAdmin && (
        <div className="mb-8 p-4 bg-red-50 border border-red-100 rounded-xl flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-red-600 mt-1">You can view these permissions but cannot modify them.</p>
          </div>
        </div>
      )}
      ...
      <div className="space-y-6">
        {rolesData.map((role) => (
          <div key={role.name} className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
            ...
            <div className="p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {role.permissions.map((perm) => (
                  <div key={perm.name} className="flex items-center justify-between p-3 rounded-lg border border-slate-100">
