    ...
    <form onSubmit={handleSubmit(onSubmit)} className="p-8">
      <div className="flex justify-between items-center border-b border-border pb-6 mb-8">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Configure global security settings for the platform.</p>
        </div>
        {isSuperAdmin && (
          <button 
            type="submit"
      ...
      <div className="space-y-8 max-w-3xl opacity-100 transition-opacity" style={{ opacity: !isSuperAdmin ? 0.7 : 1 }}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            <label className="block text-sm font-medium text-muted-foreground mb-2">Password Policy</label>
            <select 
        ...
        <div className="border-t border-border pt-8">
          <div className="flex items-center justify-between p-4 bg-muted border border-border rounded-xl">
            <div>
              <h4 className="text-sm font-medium text-foreground">Enforce Two-Factor Authentication (2FA)</h4>
              <p className="text-xs text-muted-foreground mt-1">Require all admins to set up 2FA via authenticator app.</p>
            </div>
