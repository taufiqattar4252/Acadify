...
export default function ProfilePage() {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 pb-12">
      {/* Header Banner */}
      <div className="bg-white rounded-[2rem] shadow-[0_2px_20px_-4px_rgba(0,0,0,0.05)] border border-slate-100 mb-8 overflow-hidden">
        {/* Top Gradient */}
        ...
        <div className="relative px-6 pb-8 md:px-10 flex flex-col xl:flex-row items-center xl:items-start xl:justify-between">
          ...
          <div className="relative -mt-16 md:-mt-20 flex flex-col items-center xl:items-start xl:flex-row xl:gap-6 z-10 w-full xl:w-auto">
            <div className="relative">
              <div className="w-32 h-32 md:w-40 md:h-40 rounded-full bg-white p-2 shadow-sm">
                <div className="w-full h-full rounded-full bg-emerald-50 flex items-center justify-center text-emerald-600 text-5xl font-bold overflow-hidden relative">
                  {user?.avatar ? (
            ...
            <div className="mt-4 xl:mt-20 text-center xl:text-left">
              <h1 className="text-2xl md:text-[28px] font-black text-slate-900 tracking-tight">{user?.fullName}</h1>
              <p className="text-slate-500 font-medium text-sm md:text-base mt-0.5">MHT-CET Aspirant</p>
              <div className="flex items-center justify-center xl:justify-start gap-1.5 mt-2 text-slate-500 text-xs md:text-sm font-medium">
                <MapPin className="w-4 h-4 text-emerald-500" /> Maharashtra, India
              </div>
            </div>
          </div>
          ...
          <div className="mt-8 xl:mt-6 w-full xl:w-auto flex flex-col md:flex-row items-center gap-6 xl:gap-10 justify-center">
            ...
            <div className="flex items-center gap-6 xl:gap-8">
              <div className="hidden xl:block h-16 w-px bg-slate-200"></div>
              
              <div className="flex flex-col items-center xl:items-start">
                <Crosshair className="w-5 h-5 md:w-6 md:h-6 text-emerald-500 mb-2" />
                <span className="text-[10px] md:text-[11px] text-slate-400 font-medium uppercase tracking-wider mb-0.5">Mock Tests</span>
                <span className="text-lg md:text-xl font-bold text-emerald-600">{stats?.purchasedMocks || 0}</span>
              </div>
              
              <div className="h-10 w-px bg-slate-200"></div>
              
              <div className="flex flex-col items-center xl:items-start">
                <TrendingUp className="w-5 h-5 md:w-6 md:h-6 text-emerald-500 mb-2" />
                <span className="text-[10px] md:text-[11px] text-slate-400 font-medium uppercase tracking-wider mb-0.5">Performance</span>
                <span className="text-lg md:text-xl font-bold text-emerald-600">{stats?.accuracy || 0}%</span>
              </div>
              
              <div className="h-10 w-px bg-slate-200"></div>
              
              <div className="flex flex-col items-center xl:items-start">
                <Calendar className="w-5 h-5 md:w-6 md:h-6 text-emerald-500 mb-2" />
                <span className="text-[10px] md:text-[11px] text-slate-400 font-medium uppercase tracking-wider mb-0.5">Member Since</span>
            ...
            {/* Motivation Box */}
            <div className="bg-emerald-50/80 rounded-2xl p-4 md:p-5 border border-emerald-100 flex items-center gap-4 min-w-[220px]">
              <div className="w-12 h-12 rounded-full bg-emerald-100/70 border border-emerald-200 flex items-center justify-center text-emerald-600">
                <Award className="w-6 h-6" />
              </div>
              <div>
                <p className="text-[11px] md:text-xs text-slate-500 font-medium">Keep Learning,</p>
                <p className="text-sm md:text-base font-bold text-emerald-700 leading-tight">Keep Growing!</p>
              </div>
            </div>
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
            <h3 className="font-bold text-slate-900 mb-5 flex items-center gap-2">
              <User className="w-5 h-5 text-emerald-500" /> Account Info
            </h3>
          ...
          {stats && (
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
              <h3 className="font-bold text-slate-900 mb-5 flex items-center gap-2">
                <TrendingUp className="w-5 h-5 text-emerald-500" /> Statistics
              </h3>
              
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 bg-slate-50 rounded-xl border border-slate-100">
                  <p className="text-xs font-medium text-slate-500 mb-1">Mocks Owned</p>
                  <p className="text-xl font-bold text-slate-900">{stats.purchasedMocks}</p>
                </div>
                <div className="p-4 bg-slate-50 rounded-xl border border-slate-100">
                  <p className="text-xs font-medium text-slate-500 mb-1">Total Spent</p>
                  <p className="text-xl font-bold text-slate-900">₹{stats.totalAmountSpent}</p>
                </div>
                <div className="p-4 bg-emerald-50 rounded-xl border border-emerald-100">
                  <p className="text-xs font-medium text-emerald-600 mb-1">Avg Score</p>
                  <p className="text-xl font-bold text-emerald-700">{stats.averageScore}</p>
                </div>
                <div className="p-4 bg-indigo-50 rounded-xl border border-indigo-100">
                  <p className="text-xs font-medium text-indigo-600 mb-1">Accuracy</p>
                  <p className="text-xl font-bold text-indigo-700">{stats.accuracy}%</p>
                </div>
              </div>
            </div>
          )}
            ...
            <div className="space-y-6">
              {recentActivity.length > 0 ? recentActivity.map((activity, index) => (
                <div key={activity.id + index} className="flex gap-4 relative">
                  ...
                  <div className="w-8 h-8 rounded-full bg-slate-50 border border-slate-200 flex items-center justify-center flex-shrink-0 z-10">
                    {activity.type === 'Login' && <User className="w-3 h-3 text-slate-400" />}
                    {activity.type === 'Purchase' && <CreditCard className="w-3 h-3 text-emerald-500" />}
                    {activity.type === 'Exam' && <BookOpen className="w-3 h-3 text-indigo-500" />}
                  </div>
                  <div>
                    <p className="text-sm font-semibold text-slate-900">{activity.title}</p>
                    <p className="text-xs text-slate-500">{activity.target}</p>
                    <p className="text-[10px] text-slate-400 mt-1 font-medium">{new Date(activity.date).toLocaleDateString()} at {new Date(activity.date).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</p>
                  </div>
                </div>
              )) : (
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 lg:p-8">
            <h2 className="text-xl font-bold text-slate-900 mb-6 flex items-center gap-2">
              <Save className="w-5 h-5 text-emerald-500" /> Edit Profile
            </h2>
            ...
            <form onSubmit={handleProfileSubmit(onProfileSubmit)} className="space-y-6">
              ...
              <div className="pt-6 border-t border-slate-100 mt-6">
                <h3 className="font-bold text-slate-900 mb-4 flex items-center gap-2 text-md">
                  <Crosshair className="w-4 h-4 text-emerald-500" /> Target Settings
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <Input 
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 lg:p-8">
            ...
            <div className="space-y-4 max-w-2xl">
              {[
                ...
                return (
                  <div key={pref.key} className="flex items-center justify-between p-4 rounded-xl border border-slate-100 bg-slate-50">
                    <div>
