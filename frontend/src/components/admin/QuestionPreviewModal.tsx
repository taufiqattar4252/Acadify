    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-slate-900/50 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-4xl max-h-full flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="flex justify-between items-center p-6 border-b border-border bg-muted shrink-0">
          <div>
            <h3 className="text-lg font-bold text-foreground">Question Preview</h3>
            <div className="flex flex-wrap items-center gap-3 mt-1 text-sm text-muted-foreground">
              ...
              {question.pyqYears && question.pyqYears.length > 0 && (
                <div className="flex flex-wrap gap-1">
                  {question.pyqYears.map((year: number) => (
                    <span key={year} className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-primary-light text-primary border border-purple-200">
                      PYQ {year}
                    </span>
                  ))}
                </div>
              ...
              <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium 
                ${question.difficulty === 'Easy' ? 'bg-success-light text-success' : 
                  question.difficulty === 'Medium' ? 'bg-warning-light text-warning' : 'bg-destructive-light text-destructive'}`}>
                {question.difficulty}
              </span>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 md:p-8 space-y-8 bg-white">
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                {question.questionImage && (
                  <div className="mt-4 rounded-xl overflow-hidden border border-border inline-block max-w-full">
            ...
            {question.options.map((opt: any, idx: number) => (
              <div 
                ...
                className={`relative flex items-start gap-3 p-4 rounded-xl border-2 transition-all
                  ${opt.isCorrect ? 'border-green-500 bg-success-light/30' : 'border-border bg-white'}`}
              >
                <div className={`shrink-0 w-6 h-6 rounded-full border-2 flex items-center justify-center text-xs font-bold mt-0.5
                  ${opt.isCorrect ? 'border-green-500 text-success' : 'border-border text-muted-foreground'}`}
                >
                  {String.fromCharCode(65 + idx)}
                </div>
                <div className="flex-1">
                  <p className="text-foreground font-medium whitespace-pre-wrap">{opt.text}</p>
                  {opt.image && (
                    <div className="mt-3 rounded-lg overflow-hidden border border-border inline-block max-w-sm">
          ...
          {(question.explanation || question.explanationImage) && (
            <div className="mt-8 pl-12">
              <div className="p-5 rounded-xl border border-blue-100 bg-primary-light/50 space-y-3">
        ...
        {/* Footer Actions */}
        <div className="p-6 border-t border-border bg-muted flex justify-end shrink-0">
